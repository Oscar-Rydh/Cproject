...
void* malloc(size_t size){
    ...
    */
    size_t final_size = (size + sizeof(size_t)) & ~(sizeof(size_t) - 1);
    ...
    //}
    // Allocate new memory
    void* new_memory_address = (list_t*) sbrk(final_size);
...
void free(void* ptr) {
    char* pointer_to_data = (char *) ptr;
    char* pointer_to_list = ptr - sizeof(list_t);
    printf("The pointer to the list to free is: %d\n", pointer_to_list);
    list_t* to_be_freed = (list_t*) pointer_to_list;
    ...
    if (freed == NULL) {
        ...
        current_free->next = to_be_freed;
    }

    //Merging free blocks
    list_t* free_iterator = head;
    while (free_iterator->next != NULL) {
        printf("Pointer to current is: %d \t", (char*)free_iterator);
        printf("Pointer to next is: %d \t", free_iterator->next);
        printf("current size is: %d\t", free_iterator->size);
        printf("Calculated next is: %d\n", (char*) free_iterator + 4 +  free_iterator->size);
    if ((char*) free_iterator->next == (char*) free_iterator + 4 + free_iterator->size ) {
            free_iterator->size = free_iterator->size + sizeof(list_t) + free_iterator->next->size;
    }
